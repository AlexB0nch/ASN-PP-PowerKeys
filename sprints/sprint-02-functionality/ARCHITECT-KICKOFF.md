...
**S02-006 — polish UX для `support=None`:**
- 9 команд уже wired с явными сообщениями (разбросаны по `case` в `runCommand.ts`).
- `App.tsx`: бейджи есть, но нет легенды; None-команды кликабельны и показывают красный «Error» вместо «Not on Web».
- Цель: централизованный реестр сообщений, warning-стиль для ожидаемой деградации, легенда бейджей.
