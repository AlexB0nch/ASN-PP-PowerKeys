...
1. [x] `AlignLeftToRight`, `AlignRightToLeft`, `AlignTopToBottom`, `AlignBottomToTop` **не** попадают в default «not wired up yet»; выравнивают non-anchor фигуры к соответствующему ребру anchor.
2. [x] `CopyAndAlignLeft/Right/Top/Bottom` **не** попадают в default «not wired up yet»; создают клон(ы) и выравнивают по соответствующей оси.
3. [x] Математика edge-align — в `LayoutEngine`; покрыта юнит-тестами (≥1 тест на команду).
4. [x] Edge-align команды помечены `ServerLayout` в `CommandCatalog`; `Catalog_LayoutCommandsAreMarkedServerLayout` проходит.
5. [x] Понятные ошибки: нет выделения, < 2 фигур для align-команд.
6. [x] `dotnet test PptPowerKeys.sln` — зелёный (55 passed).
7. [x] `npm run typecheck`, `npm run validate:prod` — зелёные.
8. [x] PR #19: ветка `cursor/S02-004-alignment-hostscript-51de`, Task ID `S02-004`.

## Приёмка (architect, 2026-06-28)
- PR #19, ветка `cursor/S02-004-alignment-hostscript-51de`, merge commit `2d57742` — **смержен в `main`**.
- Локально повторены `dotnet test` (55 passed), `npm run typecheck`, `npm run validate:prod` — зелёные.
- CHECKLIST: scope соблюдён, layout-математика в Core, anchor semantics сохранены, VstoLegacy не тронут.
