...
- [x] **79/79** commands routed — `CommandRouter.Execute` не бросает для Settings ids
- [x] WPF task pane: profile, snap, addup mode, shortcuts editor, Save/Reset/Export/Import
- [x] Import uses `UserSettingsImporter`; Export shape matches Web v1 (`schemaVersion`, camelCase)
- [x] Ribbon `grpSettings`: Shortcuts, Colors, Reset → 3 Settings commands
- [x] `OpenShortcutManager` scrolls to shortcuts; `ResetToDefaults` resets + reloads
- [x] `OpenColorScheme` shows pane with Colors placeholder (full UI — S10-005)
- [x] Unit tests + `dotnet test PptPowerKeys.sln` green
- [x] PR with Task ID S10-004, `Closes #99`
