...
- **Add-in (task pane):** `cd src/PptPowerKeys.AddIn`, затем `npm ci`; dev-сервер `npm start`
  (https://localhost:3000, self-signed); проверки `npm run typecheck`, `npm run validate` (dev-манифест),
  `npm run validate:prod` (production-манифест), `npm run build` / `npm run build:prod`.
  Для PowerPoint Online нужен production bundle с публичными URL — см. `docs/migration/02-powerpoint-web-deploy.md`.
...
- **Какой манифест грузить:** для **PowerPoint Online и Desktop** — закоммиченный `src/PptPowerKeys.AddIn/manifest.prod.xml`
  (публичные URL, prod-Id `5b0ca36f-...`); Windows runbook — `docs/migration/03-powerpoint-desktop-windows.md`. `manifest.xml`/`manifest.dev.xml` —
  localhost, ТОЛЬКО для dev. Файл детерминированно генерируется из `manifest.template.xml`, CI ловит дрейф
  (`build:manifest` + `git diff --exit-code`). Подробности — `docs/migration/02-powerpoint-web-deploy.md`.
