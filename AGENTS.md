...
- **Какой манифест грузить:** для **PowerPoint Online** — только закоммиченный `src/PptPowerKeys.AddIn/manifest.prod.xml`
  (публичные URL, prod-Id `5b0ca36f-...`); он доступен сразу после `git pull`. `manifest.xml`/`manifest.dev.xml` —
